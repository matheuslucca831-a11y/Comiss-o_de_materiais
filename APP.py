...
with aba3:
    ...
    # --- 1. ÁREA DE NOVO MATERIAL (DESIGN COMPACTO) ---
    with st.expander("➕ Cadastrar Novo Material no Sistema", expanded=False):
    ...
    # --- 2. BUSCA E CONTROLE DE LISTAGEM ---
    c_busca, c_stats = st.columns([2, 1])
    busca_material = c_busca.text_input("🔎 Pesquisar no Catálogo...", placeholder="Digite para filtrar...")
    
    # Busca dados
    materiais_raw = supabase.table("materiais").select("*").order("nome").execute().data
    if busca_material:
        materiais = [m for m in materiais_raw if busca_material.lower() in m["nome"].lower()]
    else:
        materiais = materiais_raw

    c_stats.metric("Total de Itens no Catálogo", len(materiais_raw))

    # --- 3. LISTAGEM ESTILIZADA ---
    if not materiais:
        st.info("Nenhum material encontrado para esta busca.")
    else:
        # Cabeçalho da Lista
        h1, h2, h3 = st.columns([6, 1, 1])
        h1.caption("📦 NOME DO MATERIAL")
        h2.caption("📝 EDITAR")
        h3.caption("🗑️ EXCLUIR")

        for m in materiais:
            # Container para cada linha para dar um aspecto de card
            with st.container(border=True):
                col1, col2, col3 = st.columns([6, 1, 1])
                
                with col1:
                    st.markdown(f"**{m['nome'].upper()}**")
                
                with col2:
                    if st.button("✏️", key=f"edit_mat_{m['id']}", help="Editar nome"):
...
with aba4:
    ...
    # Seleção de Unidade e Ambiente (Fora do form para manter a reatividade)
    c1, c2 = st.columns(2)
    ...
    if unidade_sel:
        ...
        if ambiente_sel:
            ...
            with st.form("form_cadastro_detalhes", clear_on_submit=True):
                ...
                if btn_salvar:
                    # --- LÓGICA DE SALVAMENTO COM FEEDBACK ---
                        ...
                        try:
                            ...
                            if material_sel and material_sel["id"] == "outro":
                                if not novo_material:
                                    ...
                                    nome_limpo = novo_material.strip()
                                    res_mat = supabase.table("materiais").upsert(
                                        {"nome": nome_limpo}, on_conflict="nome"
                                    ).execute()
                            ...
                            if material_id:
                                ...
                                user_atual = st.session_state.get("nome_admin", "Usuário Desconhecido")
...
# --- NO FINAL DO ARQUIVO (FORA DAS ABAS) ---

# 1. Função Única para gerenciar todos os diálogos do sistema
def gerenciador_de_dialogos():
    # Verifica qual ação foi disparada
    ...
    if st.session_state.get("confirm_delete_ambiente"):
        ...
        def modal():
            ...
            if st.button("Confirmar", type="primary", use_container_width=True):
                ...
                st.cache_data.clear()
                st.session_state.pop("confirm_delete_ambiente") # Remove a chave
                st.rerun()
    ...
    # --- MATERIAIS ---
        ...
        def modal():
            ...
            st.warning(f"Isso apagará todos os registros de {mat['nome']}.")
